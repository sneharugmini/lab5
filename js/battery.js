...
// STEP 1d: Grab the image element to display the generated image
...
function updateBatteryStatus(battery) {
    ...
    // STEP 5: Fetch image from Robohash API based on battery percentage
    fetch(`https://robohash.org/${Math.floor(battery.level * 100)}?set=set4`)
        .then(response => {
            if (!response.ok) {
